...
my $usage= << "JUS";
  ...
  results: Branch and bound algorithm for fiven input File and defined cutoff. returns all found solutions for given cutoff,
           verbose prints also all not used paths 
JUS
...
my $best= 9**9**9;  # should be infinity, or so, should check again

my $curPos="";
my $cost=0;
my %solution=();
while (scalar @stack != 0) { # as long as there are things on the stack
    $curPos = pop(@stack); # get the last element from stack, is now a string with the city names till this branch
    $cost=cost($curPos);
    if ($cost<=$opt_cutoff)
    {
        #print "A= $A\tCost= $cost\n";
